...
show_scoreboard = False #scoreboard: detected bags for each color - fps text
...
#0-9 = R1, 10-19 = R2, 20-29 = R3, 30-39 = R4, 40-49 = B1, 50-57 = B2, 58-67 = B3, 68+ = B4
id_array = [
    "8157123",
    "13854",
    "173638",
...
while True:
    ...
    if(GPIO.input(23) == False and GPIO.input(24) == False and GPIO.input(25) == True):
        #1 - nothing (TFF)
        #make FFT
        ...
        track_b_min.update(0)
        track_reset.update(0)
    elif(GPIO.input(23) == True and GPIO.input(24) == False and GPIO.input(25) == False):
        #7 - Advance (TTT)
        #make TFF
        track_r_plus.update(0)
        track_r_min.update(0)
    ...
    if(received_data):
        arr = np.frombuffer(received_data, dtype=np.uint8)
        if(len(arr) == 5):
            arr = np.delete(arr, [2, 3])
            string_val = ''.join(str(num) for num in arr)
            if(not(string_val.startswith("147"))):
                if(string_val in id_array):
                    index = id_array.index(string_val)
                    if(bag_num_red(index) != -1):
                        red_RFID[bag_num_red(index) - 1] = 1
                    elif(bag_num_blue(index) != -1):
                        blue_RFID[bag_num_blue(index) - 1] = 1
    ...
    '''SCORE CALC AND OUTPUT'''
    # region SCORE CALC AND OUTPUT
